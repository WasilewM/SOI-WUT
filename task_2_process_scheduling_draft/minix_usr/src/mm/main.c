...
/* SOI LAB 2 */
extern int PROCENT_0 = 25;
extern int PROCENT_1 = 75;
 ...
 *				do_getprocnr					     *
 *===========================================================================*/
 PUBLIC int do_getprocnr(int proc_nr)
 {
    int look_for_proc = mm_in.m1_i1, p_nr = 0;
    pid_t current_proc;
    
    for (p_nr = 0; p_nr < NR_PROCS; p_nr++)
    {
    	if ((mproc[p_nr].mp_flags & IN_USE) && (mproc[p_nr].mp_pid == look_for_proc))
    	{
    	    return p_nr;
    	}
    }
 ...
 *				do_changegroup					     *
 *===========================================================================*/
 PUBLIC int do_changegroup(int proc_nr)
 {
 
    int look_for_proc = mm_in.m1_i1, p_nr = 0, current_group = 0;
    pid_t current_proc;
    
    /* struct proc *p; */
    
    /*
    Used this line to check if procedure was called properly
    printf("INSIDE do_changegroup\n");
    */
    
    /* find proc with given pid */
 /*
    for (p_nr = 0; p_nr < NR_PROCS; p_nr++)
    {
    	if ((mproc[p_nr].mp_flags & IN_USE) && (mproc[p_nr].mp_pid == look_for_proc))
    	{
    	    printf("process with pid %d at %u\n", look_for_proc p_nr);
    	}
    }
 */
 ...
 *				do_changeproc					     *
 *===========================================================================*/
 PUBLIC int do_changeproc(int proc_nr)
 {
    int new_val = mm_in.m1_i1;
    
    /*
    Used this line to check if procedure was called properly
    printf("INSIDE do_changeproc\n");
    */
    ...
    if(new_val > 0 && new_val < 50) {
    	PROCENT_0 = new_val;
    	PROCENT_1 = 100 - PROCENT_0;
    }
    else if (new_val > 50 && new_val < 99) {
    	PROCENT_1 = new_val;
    	PROCENT_0 = 100 - PROCENT_1;
    }
