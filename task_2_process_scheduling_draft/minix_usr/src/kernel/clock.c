...
/* Global variables - PROCENT_0 + PROCENT_1 must be equal to 100*/
/*extern int PROCENT_0 = 25;
extern int PROCENT_1 = 75;*/
int PROCENT_0, PROCENT_1;
 ...
 */
  ...
  if (--sched_ticks == 0) {
	if (bill_ptr == prev_ptr) lock_sched();	/* process has run too long */
		if (rdy_head[USER_Q]->p_group == 0)
		{
		  sched_ticks = SCHED_RATE * PROCENT_0;
		}
		else
		{
		  sched_ticks = SCHED_RATE * PROCENT_1;
		}
