...
PRIVATE message m;
PUBLIC int next_process_group=0;
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  /* SOI - LAB 2 */
  if (next_process_group == 0)
  {
    /* next_process_group == 0 -> GROUP_0 */
    rpc->p_group = GROUP_0;
  }
  else
  {
    /* next_process_group == 1, 2, 3 -> GROUP_1 */
    rpc->p_group = GROUP_1;
  }
  next_process_group = (next_process_group + 1) % 4;
  return(OK);
}
