...
extern int is_worst_fit_active = 0;	/* SOI LAB 5 */
...
if (is_worst_fit_active) {
	/* worst fit is active - choose the largest hole */
	...
	/* worst fit is not active - choose first hole that is large enough */
 ...
 *				do_worst_fit					     *
 *===========================================================================*/
 PUBLIC int do_worst_fit( int new_val )
 {
 	
 	int change_for_val = mm_in.m1_i1;
 	
 	if (change_for_val != 0) {
 		is_worst_fit_active = 0;
 	}
 	else {
 		is_worst_fit_active = 1;
 	}
 ...
 *===========================================================================*/
 PUBLIC int do_hole_map( void )
 {
	...
	while ((hp != NIL_HOLE) && (hp->h_base < swap_base) && (idx + 2 + 1 <= buffer_size)) {
	
		buffer[idx] = hp->h_len;
		idx++;
		buffer[idx] = hp->h_base;
		idx++;
		
		hp = hp->h_next;	
	}
	
	buffer[idx] = 0;	/* add last item into buffer - must be 0 */
